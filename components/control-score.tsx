...
const ControlScore = ({ stools }: { stools: Stool[] | null }) => {
  // スコア計算
  const score =
    stools && stools.length > 0
      ? stools.reduce((total: number, stool: Stool) => {
          const bristolScore = 100 - Math.abs(stool.scale - 4) * 20
          const volumeScore = 100 - Math.abs(stool.volume - 50) * 2
          const colorScore = stool.color >= 3 && stool.color <= 4 ? 100 : 70
          return (
